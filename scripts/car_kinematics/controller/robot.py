...
class Robot:
    def __init__(self):
        ...
        self.cur_throttle_microseconds = Float32()
        self.cur_steering_microseconds = Float32()
        self.cur_throttle_microseconds.data = 0.0
        self.cur_steering_microseconds.data = 0.0
        
        self.pub_microseconds_throttle = \
            rospy.Publisher('/throttle', Float32, queue_size=10)
        self.pub_microseconds_steering = \
            rospy.Publisher('/steering', Float32, queue_size=10)
        self.pub_microseconds_throttle.publish(self.cur_throttle_microseconds)
        self.pub_microseconds_steering.publish(self.cur_steering_microseconds)

        
    def shutdown(self):
        rospy.loginfo(rospy.get_caller_id() + " Robot shutdown")
        self.cur_throttle_microseconds.data = 1500.0
        self.cur_steering_microseconds.data = 1500.0
        self.pub_microseconds_throttle.publish(self.cur_throttle_microseconds)
        self.pub_microseconds_steering.publish(self.cur_steering_microseconds)
    ...
    def set_speed(self, throttle, steering):
        # Clamp the wheel speeds to actuator limits
        # throttle = max(min(throttle, self.wheel_speed_max), self.wheel_speed_max * -1.0)
        # steering = max(min(steering, self.wheel_speed_max), self.wheel_speed_max * -1.0)

        # Convert to power norms
        self.cur_throttle_microseconds.data = 1500 + throttle * 110# 69 for crawler
        self.cur_steering_microseconds.data = 1500 + steering * 200

        # Publish out
        if self.cur_throttle_microseconds.data != 0.0 or \
           self.cur_steering_microseconds.data != 0.0:
            rospy.loginfo(rospy.get_caller_id() +
                          " throttle microseconds: " + str(self.cur_throttle_microseconds) +
                          " steering microseconds: " + str(self.cur_steering_microseconds))
        self.pub_microseconds_throttle.publish(self.cur_throttle_microseconds)
        self.pub_microseconds_steering.publish(self.cur_steering_microseconds)
